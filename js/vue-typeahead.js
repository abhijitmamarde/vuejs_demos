const MyTypeAheadComponent = {
  ...
  watch: {
    local: function(newVal) {
      if(this.defaultSuggestion) {
        ...
        // AJM
        this.defaultSuggestions = newVal.slice();
      }
      this.resetTypeahead();
  ...
  mounted: function() {
    ...
    if(this.local.length) {
      ...
      // AJM
      this.defaultSuggestions = this.local.slice();
    }
  },
  ...
  methods: {
    ...
    bloodhoundOption: function(){
      ...
      if(this.prefetch) {
        ...
        if(this.defaultSuggestion) {
          ...
          // AJM
          var prefetch_old = prefetch
          console.log('prefetch:')
          console.log(prefetch)
          prefetch = {
            cache: prefetch_old.cache,
            url: prefetch_old.url,
      ...
      if(this.local) {
        ...
        // }
        // AJM
        bloodhoundConfig = $.extend({local: this.local}, bloodhoundConfig)
      }
      if(this.remote) {
        // bloodhoundConfig = {
        //   remote: {
    ...
    getSource: function(){
      ...
      // AJM
      var new_bloodhoundConfig = $.extend({
        datumTokenizer,
        queryTokenizer: Bloodhound.tokenizers.whitespace
      }, bloodhoundConfig)

      var engine = new Bloodhound(new_bloodhoundConfig)
